...
CREATE OR REPLACE TRANSIENT TABLE FIN_TRADE_EXTRACT.RAW.EARNINGS_CALL_TRANSCRIPT_STAGING AS
CREATE OR REPLACE TRANSIENT TABLE EARNINGS_CALL_TRANSCRIPT_STAGING AS
SELECT * FROM FIN_TRADE_EXTRACT.RAW.EARNINGS_CALL_TRANSCRIPT WHERE 1=0;
COPY INTO FIN_TRADE_EXTRACT.RAW.EARNINGS_CALL_TRANSCRIPT_STAGING
COPY INTO EARNINGS_CALL_TRANSCRIPT_STAGING
FROM @FIN_TRADE_EXTRACT.RAW.EARNINGS_CALL_TRANSCRIPT_STAGE
FROM @EARNINGS_CALL_TRANSCRIPT_STAGE
FILE_FORMAT = (TYPE = 'CSV');
MERGE INTO FIN_TRADE_EXTRACT.RAW.EARNINGS_CALL_TRANSCRIPT AS target
MERGE INTO FIN_TRADE_EXTRACT.RAW.EARNINGS_CALL_TRANSCRIPT AS target
USING (
    SELECT 
        s.SYMBOL,
