...
USING (
    SELECT 
        ...
        "reported_currency" AS REPORTED_CURRENCY,
        
        -- Assets
        TRY_TO_NUMBER("total_assets", 38, 2) AS TOTAL_ASSETS,
        TRY_TO_NUMBER("total_current_assets", 38, 2) AS TOTAL_CURRENT_ASSETS,
        TRY_TO_NUMBER("cash_and_cash_equivalents", 38, 2) AS CASH_AND_CASH_EQUIVALENTS,
        TRY_TO_NUMBER("cash_and_short_term_investments", 38, 2) AS CASH_AND_SHORT_TERM_INVESTMENTS,
        TRY_TO_NUMBER("inventory", 38, 2) AS INVENTORY,
        TRY_TO_NUMBER("current_net_receivables", 38, 2) AS CURRENT_NET_RECEIVABLES,
        TRY_TO_NUMBER("total_non_current_assets", 38, 2) AS TOTAL_NON_CURRENT_ASSETS,
        TRY_TO_NUMBER("property_plant_equipment", 38, 2) AS PROPERTY_PLANT_EQUIPMENT,
        TRY_TO_NUMBER("accumulated_depreciation_amortization_ppe", 38, 2) AS ACCUMULATED_DEPRECIATION_AMORTIZATION_PPE,
        TRY_TO_NUMBER("intangible_assets", 38, 2) AS INTANGIBLE_ASSETS,
        TRY_TO_NUMBER("goodwill", 38, 2) AS GOODWILL,
        TRY_TO_NUMBER("investments", 38, 2) AS INVESTMENTS,
        TRY_TO_NUMBER("long_term_investments", 38, 2) AS LONG_TERM_INVESTMENTS,
        TRY_TO_NUMBER("short_term_investments", 38, 2) AS SHORT_TERM_INVESTMENTS,
        TRY_TO_NUMBER("other_current_assets", 38, 2) AS OTHER_CURRENT_ASSETS,
        TRY_TO_NUMBER("other_non_current_assets", 38, 2) AS OTHER_NON_CURRENT_ASSETS,
        
        -- Liabilities
        TRY_TO_NUMBER("total_liabilities", 38, 2) AS TOTAL_LIABILITIES,
        TRY_TO_NUMBER("total_current_liabilities", 38, 2) AS TOTAL_CURRENT_LIABILITIES,
        TRY_TO_NUMBER("current_accounts_payable", 38, 2) AS CURRENT_ACCOUNTS_PAYABLE,
        TRY_TO_NUMBER("deferred_revenue", 38, 2) AS DEFERRED_REVENUE,
        TRY_TO_NUMBER("current_debt", 38, 2) AS CURRENT_DEBT,
        TRY_TO_NUMBER("short_term_debt", 38, 2) AS SHORT_TERM_DEBT,
        TRY_TO_NUMBER("total_non_current_liabilities", 38, 2) AS TOTAL_NON_CURRENT_LIABILITIES,
        TRY_TO_NUMBER("capital_lease_obligations", 38, 2) AS CAPITAL_LEASE_OBLIGATIONS,
        TRY_TO_NUMBER("long_term_debt", 38, 2) AS LONG_TERM_DEBT,
        TRY_TO_NUMBER("current_long_term_debt", 38, 2) AS CURRENT_LONG_TERM_DEBT,
        TRY_TO_NUMBER("long_term_debt_noncurrent", 38, 2) AS LONG_TERM_DEBT_NONCURRENT,
        TRY_TO_NUMBER("other_current_liabilities", 38, 2) AS OTHER_CURRENT_LIABILITIES,
        TRY_TO_NUMBER("other_non_current_liabilities", 38, 2) AS OTHER_NON_CURRENT_LIABILITIES,
        
        -- Equity
        TRY_TO_NUMBER("total_shareholder_equity", 38, 2) AS TOTAL_SHAREHOLDER_EQUITY,
        TRY_TO_NUMBER("treasury_stock", 38, 2) AS TREASURY_STOCK,
        TRY_TO_NUMBER("retained_earnings", 38, 2) AS RETAINED_EARNINGS,
        TRY_TO_NUMBER("common_stock", 38, 2) AS COMMON_STOCK,
        TRY_TO_NUMBER("common_stock_shares_outstanding", 38, 0) AS COMMON_STOCK_SHARES_OUTSTANDING,
