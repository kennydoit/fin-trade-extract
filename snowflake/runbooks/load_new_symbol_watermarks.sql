...
-- Destination: FIN_TRADE_EXTRACT.RAW.ETL_WATERMARKS_TEST (for testing)
...
-- Step 0: Create test table as clone of production (for dry-run testing)
CREATE OR REPLACE TABLE FIN_TRADE_EXTRACT.RAW.ETL_WATERMARKS_TEST 
CLONE FIN_TRADE_EXTRACT.RAW.ETL_WATERMARKS;
...
WHERE SYMBOL NOT IN (
    ...
    FROM FIN_TRADE_EXTRACT.RAW.ETL_WATERMARKS_TEST
);
...
-- Step 3: Cross join new symbols with all table names
CREATE OR REPLACE TEMPORARY TABLE NEW_SYMBOLS_TABLES AS
SELECT 
    t.TABLE_NAME,
    s.SYMBOL_ID,
    s.SYMBOL,
...
INSERT INTO FIN_TRADE_EXTRACT.RAW.ETL_WATERMARKS_TEST (
    TABLE_NAME,
    SYMBOL_ID,
    SYMBOL,
    NAME,
