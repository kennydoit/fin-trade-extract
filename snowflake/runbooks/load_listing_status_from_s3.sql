...
FROM @LISTING_STATUS_STAGE/listing_status_20251002.csv 
(FILE_FORMAT => FIN_TRADE_EXTRACT.RAW.RAW_CSV_FORMAT) 
LIMIT 5;

-- Use a named file format for compatibility
COPY INTO FIN_TRADE_EXTRACT.RAW.LISTING_STATUS_STAGING (symbol, name, exchange, assetType, ipoDate, delistingDate, status)
...
SELECT COUNT(*) as rows_loaded FROM FIN_TRADE_EXTRACT.RAW.LISTING_STATUS_STAGING;
...
SELECT * FROM FIN_TRADE_EXTRACT.RAW.LISTING_STATUS_STAGING LIMIT 5;
