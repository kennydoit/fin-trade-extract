-- ============================================================================
-- ALERTING SYSTEM: Automated Monitoring and Notifications
-- Creates stored procedures for automated alerts on data quality and freshness
-- =====================================BEGIN
    -- Run all alert checks
    SELECT ALERT_COUNT INTO freshness_alerts FROM TABLE(CHECK_DATA_FRESHNESS_ALERTS());
    SELECT ALERT_COUNT INTO quality_alerts FROM TABLE(CHECK_DATA_QUALITY_ALERTS());
    SELECT ALERT_COUNT INTO volume_alerts FROM TABLE(CHECK_VOLUME_ANOMALY_ALERTS());==================================
...
BEGIN
    ...
    RETURN TABLE(SELECT alert_count);
END;
$$;
...
DECLARE
    ...
    freshness_alerts INTEGER;
    quality_alerts INTEGER;
    volume_alerts INTEGER;
    result_message STRING;
BEGIN
    -- Run all alert checks
    CALL CHECK_DATA_FRESHNESS_ALERTS() INTO freshness_alerts;
    CALL CHECK_DATA_QUALITY_ALERTS() INTO quality_alerts;
    CALL CHECK_VOLUME_ANOMALY_ALERTS() INTO volume_alerts;
    
    SET total_alerts = freshness_alerts + quality_alerts + volume_alerts;
    
    SET result_message = 'Alert check completed. Total alerts triggered: ' || total_alerts || 
                        ' (Freshness: ' || freshness_alerts || 
...
SELECT 'ALERTING SYSTEM COMPLETE' as status;
